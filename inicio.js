...
if (
  ...
  const productDots = Array.from(
    productDotsContainer.querySelectorAll(".catalog-product-dot"),
  );
  ...
  function getVisibleProducts() {
    ...
    return Math.max(1, Math.min(amount, totalProducts));
  }
  ...
  function getCurrentProductIndex() {
    ...
    return Number.parseInt(firstCard.dataset.productIndex, 10);
  }
  ...
  function processMovementQueue() {
    if (isAnimating || movementQueue.length === 0) {
      return;
    }
  ...
  function moveProductNext() {
    ...
    productTrack.style.transform = `translateX(-${step}px)`;
  ...
  function moveToProduct(targetIndex) {
    ...
    const forwardDistance =
      (targetIndex - currentIndex + totalProducts) %
      totalProducts;

    const backwardDistance =
      (currentIndex - targetIndex + totalProducts) %
      totalProducts;
...
const featureImage = document.getElementById(
  "catalog-feature-image",
);
const featureBrand = document.getElementById(
  "catalog-feature-brand",
);
const featureTitle = document.getElementById(
  ...
  productCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      window.setTimeout(() => {
        ...
        featureImage.alt =
          card.dataset.featureTitle || "Producto destacado";
